...
int set_ALSA_volume(snd_mixer_elem_t * elem, int step, int val)
{
	...
	for (int i = 0; i < nch; i++) {
		err =
		    snd_mixer_selem_get_playback_dB(elem, chlist[i], &chval[i]);
		if (err) {
			ERR("ALSA error getting value for %s ch %d: %s.",
			    snd_mixer_selem_get_name(elem), i,
			    snd_strerror(err));
		} else {
		...
		err =
		    snd_mixer_selem_set_playback_dB(elem, chlist[i], chval[i],
						    val);
...
int set_ALSA_mute(snd_mixer_elem_t * elem, int val)
{
	...
	for (int i = 0; i < nch; i++) {
		err =
		    snd_mixer_selem_get_playback_switch(elem, chlist[i],
							&chval[i]);
		if (err) {
			ERR("ALSA error getting value for %s ch %d: %s.",
			    snd_mixer_selem_get_name(elem), i,
			...
			DBG("ALSA reports current setting for %s ch %d as %d.",
			    snd_mixer_selem_get_name(elem), i, chval[i]);
		}
		chval[i] = val;
		err =
		    snd_mixer_selem_set_playback_switch(elem, chlist[i],
							chval[i]);
		/* This call creates spurious error conditions even though it succeeds. Deactivate error reporting until fixed.
		   if (err) {
